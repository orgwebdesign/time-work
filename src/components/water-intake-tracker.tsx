...
export function WaterIntakeTracker() {
  ...
  const goalMet = useMemo(() => count >= DAILY_GOAL, [count]);
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-center gap-4 text-center cursor-pointer" onClick={handleIncrement}>
            ...
            <div className="flex items-baseline gap-2">
                <span className={cn("text-4xl font-bold", { 'text-green-500': goalMet })}>{count}</span>
         ...
         <div className="space-y-2">
             ...
             <p className="text-xs text-center text-muted-foreground">
                {goalMet ? "Goal achieved! 🎉" : `${DAILY_GOAL - count > 0 ? `${DAILY_GOAL - count} glasses to go.` : "Let's start hydrating!"}`}
            </p>
         </div>
      </CardContent>
    </Card>
